...
- 풀이 방법
  ...
  - 문제에서 위와 같은 글귀를 보고 위상정렬을 사용해야겠다고 생각했다.
  - 주어진 입력에 따른 그래프인 graph 리스트와 이와 역방향을 가지는 그래프인 reverse_graph를 만들었다.
  - 그 후, graph리스트를 바탕으로 위상정렬을 수행하면서 indegree가 0이 되면, reverse_graph를 토해서 해당 노드와 연결된 값들 중 `result[next] = max(result[next],result[i] + weight[next][0])`를 바탕으로 값을 계산하면 해당 건물이 지어질 수 있는 가장 최소의 시간이 구해진다.
...
- 풀이 방법
  - 문제를 이해하는데 힘들긴 했다.
  - 하나의 마을에 각각의 집들이 있고, 이 하나의 마을을 두 개의 마을로 분리시키고자 한다.
  - 한 마을 안 집들은 서로 연결(이게 애매한데 즉 어떻게든 갈 수 있어야함을 의미)되어 있어야하고, 다른 마을의 집들과는 연결이 안되어 있어야한다.
  - 그래서 생각한 것이 MST(Minimum Spanning Tree)를 만들어야겠다고 생각했다.
  - MST를 만들고 난 뒤 가장 큰 유지비를 가진 길을 없애버리면 최소값을 구할 수 있다.
  - ![1647](./readme_img/1647.JPG)
    - 여기서 형광색 부분이 MST이고 이 중 가장 큰 값인 3을 제거한 나머지 길들의 합이 정답이다.
...
<details>
<summary>보석 도둑</summary>

- 링크 : https://www.acmicpc.net/problem/1202
- 풀이 방법
  - 문제를 보자마자, 그리디라는 것은 알았으나 naive하게 푸니까 시간초과를 면할 수 없었다.
  - 그래서 보석을 무게 순으로 sorting하고 가방을 무게 순으로 sorting해준다.
  - 그 후, 우선순위 큐를 만든 다음, sorting된 가방을 하나씩 검사해서 해당 가방에 들어갈수 잇는 보석들을 우선순위 큐에 넣어준다. 이때 우선순위 큐는 보석의 가치가 우선순위를 가진다.
  - 해당 가방에 들어갈 수 있는 보석을 우선순위큐에 넣은 후 우선순위큐에 원소가 있다면 하나를 꺼내서 result에 더해준다.
  - 위 로직이 마치면 result를 출력하면 된다.
...
<details>
<summary>사회망 서비스</summary>

- 링크 : https://www.acmicpc.net/problem/2533

풀이 방법

tree dp 문제이다. 아직 개념이 익숙치 않아서 인터넷에서 검색해서 품.
...
게이트에 비행기를 최대 도킹할 수 있는 수를 구해야하는 문제이다.

`이후 P개의 줄에 gi (1 ≤ gi ≤ G) 가 주어진다.` 이라고 주어지는 입력에서 disjoint set을 떠올렸다.

처음에는 parent를 자신의 인덱스로 정의한다.<br>
ex> parent = [0,1,2,3,4,...]

그 후, 첫 비행기가 원하는 게이트 번호부터 검사한다.

해당 번호(g)의 find_parent하고 만약, g와 find_parent 값이 같다면 해당 번호는 비어있으며 도킹이 가능하다는 의미이다. 그리고 만약 다음번에 들어오면 이 번호가 아닌 자신의 앞 번호 중 비어있는 게이트 중 가장 큰 숫자의 게이트로 가야한다. 그러나 만약 find_parent가 0(즉, 꽉참)일 경우, 도킹 할 수 없으니 멈춘다.
...
우선 python으로 제출할때는 시간초과가 발생해서 pypy로 제출하니까 통과했다(아직도 불통한 이유는 모르겠다...)

우선 테트로미노의 집합은 총 19개가된다.(회전,대칭 포함) 그리고, 종이의 크기가 `N*M`이며 최대 `500*500`이 된다. 그러므로 경우의수를 계산하면 `500*500*19`이므로 완전탐색을하면 되겠다고 생각했다.
...
```
- 테트로미노의 19가지 경우를 좌표로 나타낸건데 만들기 귀찮아서 인터넷에서 가져왔다.
...
```
- 테트로미노를 하나씩 검사한다.
- 0,0부터 시작해서 테트로미노를 옮겨가면서 그들의 합을 계산하고 answer에 넣어주면 된다
- 이때, 테트로미노의 좌표가 종이 보다 커지면 안되니까 이를 처리해주었다.
